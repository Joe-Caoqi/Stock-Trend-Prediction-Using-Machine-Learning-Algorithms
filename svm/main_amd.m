...
X_test  = importdata('X_test_amd.mat');
X_train = importdata('X_train_amd.mat');
y_test  = importdata('y_test_amd.mat');
y_train = importdata('y_train_amd.mat');
...
if flag
clear all; clc
load amd_processeddata.mat
amd_data = table2array(amdprocesseddata);
...
X = [];
for i = 1:121
    X = [X normalize(amd_data(:,i)','range')'];
end
X = double(X);
...
X_test_amd  = X(testInds,:);
y_test_amd  = y(testInds,:);
X_train_amd = X(trainInds,:);
y_train_amd = y(trainInds,:);

save X_test_amd X_test_amd
save y_test_amd y_test_amd
save X_train_amd X_train_amd
save y_train_amd y_train_amd
end
