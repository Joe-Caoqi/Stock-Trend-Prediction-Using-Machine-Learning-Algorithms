...
X_test  = importdata('X_test_jpm.mat');
X_train = importdata('X_train_jpm.mat');
y_test  = importdata('y_test_jpm.mat');
y_train = importdata('y_train_jpm.mat');
...
if flag
clear all; clc
load jpmprocesseddata.mat
jpm_data = table2array(jpmprocesseddata);
...
X = [];
for i = 1:124
    X = [X normalize(double(jpm_data(:,i))','range')'];
end
X = double(X);
...
X_test_jpm  = X(testInds,:);
y_test_jpm  = y(testInds,:);
X_train_jpm = X(trainInds,:);
y_train_jpm = y(trainInds,:);

save X_test_jpm X_test_jpm
save y_test_jpm y_test_jpm
save X_train_jpm X_train_jpm
save y_train_jpm y_train_jpm
end
...
%% Optimized jpmdel - Train and Test
% linear
C = 1;
SVMModel = fitcsvm(X_train,y_train,'Standardize',true,...
    'KernelFunction','linear','BoxConstraint',C);
