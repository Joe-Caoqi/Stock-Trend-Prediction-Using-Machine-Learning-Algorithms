...
X_test  = importdata('X_test_ibm.mat');
X_train = importdata('X_train_ibm.mat');
y_test  = importdata('y_test_ibm.mat');
y_train = importdata('y_train_ibm.mat');
...
if flag
clear all; clc
load ibm_processeddata.mat
ibm_data = table2array(ibmprocesseddata);
...
X = [];
for i = 1:126
    X = [X normalize(ibm_data(:,i)','range')'];
end
X = double(X);
...
X_test_ibm  = X(testInds,:);
y_test_ibm  = y(testInds,:);
X_train_ibm = X(trainInds,:);
y_train_ibm = y(trainInds,:);

save X_test_ibm X_test_ibm
save y_test_ibm y_test_ibm
save X_train_ibm X_train_ibm
save y_train_ibm y_train_ibm
end
