...
X_test  = importdata('X_test_hpq.mat');
X_train = importdata('X_train_hpq.mat');
y_test  = importdata('y_test_hpq.mat');
y_train = importdata('y_train_hpq.mat');
...
if flag
clear all; clc
load hpqprocesseddata.mat
hpq_data = table2array(hpqprocesseddata);
...
X = [];
for i = 1:122
    X = [X normalize(double(hpq_data(:,i))','range')'];
end
X = double(X);
...
X_test_hpq  = X(testInds,:);
y_test_hpq  = y(testInds,:);
X_train_hpq = X(trainInds,:);
y_train_hpq = y(trainInds,:);

save X_test_hpq X_test_hpq
save y_test_hpq y_test_hpq
save X_train_hpq X_train_hpq
save y_train_hpq y_train_hpq
end
...
%% Optimized hpqdel - Train and Test
% linear
C = 1;
SVMModel = fitcsvm(X_train,y_train,'Standardize',true,...
    'KernelFunction','linear','BoxConstraint',C);
