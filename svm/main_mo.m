...
X_test  = importdata('X_test_mo.mat');
X_train = importdata('X_train_mo.mat');
y_test  = importdata('y_test_mo.mat');
y_train = importdata('y_train_mo.mat');
...
if flag
clear all; clc
load moprocesseddata.mat
mo_data = table2array(moprocesseddata);
...
X = [];
for i = 1:121
    X = [X normalize(double(mo_data(:,i))','range')'];
end
X = double(X);

% Partition into Test and Train
SVM_Mdl_HO = fitcsvm(X,y,'Holdout',0.6,'Standardize',true);
...
X_test_mo  = X(testInds,:);
y_test_mo  = y(testInds,:);
X_train_mo = X(trainInds,:);
y_train_mo = y(trainInds,:);

save X_test_mo X_test_mo
save y_test_mo y_test_mo
save X_train_mo X_train_mo
save y_train_mo y_train_mo
end
